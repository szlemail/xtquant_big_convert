...
一次系统性的实盘安全审查后的分批修复（每项均先复现验证、再修、再回归；离线套件 523 → 591 passed），随后在国金模拟 QMT（gjqmt_test，ZMQ 无 Redis 部署）完成端到端实测：ping/行情/持仓查询/全推推送全通，实测量级 ping ~60ms、沪深A股 5217 只、K 线/快照 ~50-550ms、持仓资产查询 ~1.3s（deferred 路径）。

### 修复（实测部署补充）

- **`bigqmt-init` 向导的 ZMQ 客户端端口镜像公式与传输层实现漂移**：向导按 `15000+账号%1000` 生成客户端连接地址，服务端实际绑定 `15560+账号%100`——向导产出的 ZMQ 客户端永远连不上死端口。镜像改为直接复用传输层的 `_default_zmq_port`，并加回归测试锁定两者一致。
- **服务端首个行情 RPC 要先付原生 xtdata SDK 加载/行情服务连接超时的成本**（实测：ping 300ms 应答，行情请求全部排队撞 8s 客户端超时，数分钟后才恢复）。启动时后台守护线程预热一发 `get_trading_dates`，把该成本移出请求路径；纯尽力而为，失败全吞。
- 客户端私有配置不再放仓库根目录（污染测试 CWD、且有凭据入库风险），移至 `tools/deploy/`（已 gitignore），e2e 探测脚本自行挂载该目录。
