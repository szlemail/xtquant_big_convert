...
> ⚠️⚠️ **下单必读（2026-09-07 实测踩坑，runbook 未记载）**：
> 编辑器/模型研究里运行只提供行情与查询服务，**passorder 会被终端静默丢弃**
> （柜台零记录、资金零冻结、无任何报错——查证手段：`get_last_order_id`
> 返回空 + 委托列表无此单）。要真正发单，必须：
> 1. 策略加入 **模型交易** 实例并**绑定资金账号**；
> 2. **把实例的「模拟/实盘」切换开关设到对应模式**（国金 QMT 实测：
>    该开关在实例设置里，不选模式时委托被静默丢弃，症状与未迁实例
>    完全相同）。
> 判别标志：下单后 `query_stock_orders` 出现带自己 remark 的委托 +
> 资金出现对应冻结。两样都有才算通。
> 生产账户部署时尤其注意：同一开关切到「实盘」即真金白银，配合
> `rpc_allow_order_methods` 只在有交易意图的窗口打开。
